...
(function($) {
	...
	**/
	...
	var draw = function($picker, option, year, month, day, hour, min) {
		...
		var isPastOnly = $picker.data("pastOnly");
		...
		if (isPastOnly) {
			if (date.getTime() > todayDate.getTime()) { // Future date
				date.setTime(todayDate.getTime());
			}
		}
		...
		if ((!isPastOnly || !isCurrentMonth) && (maxDate == null) || (maxDate > cDate.getTime())) {
			$link_next_month = $('<a>');
		...
		if (isCloseButton) {
			...
			$link_close.addClass('dt-icon-close');
		...
		for (var zz = 0; i < cellNum; i++) {
			...
			else if (isPastOnly && !isPast && !isToday) { // Future day
				$td.addClass('day_in_unallowed');
			} else {
		...
		if ($picker.data("dateOnly") === true) {
			...
			$timelist.css("height", Math.max($calendar.innerHeight() - 10, 176) + 'px');
			...
			while( hour_*100+min_ < maxTime[0]*100+maxTime[1] ){
				...
				} else if (isPastOnly && isCurrentDay && !is_past_time) { // Past cell
					$o.addClass('time_in_unallowed');
				} else { // Normal cell
	...
	var init = function($obj, opt) {
		...
		$picker.data('pastOnly', opt.pastOnly);
